...
function smartadapt_setup() {
	...
	// Load external function - shortcodes
...
function smartadapt_enqueue_admin_libraries() {
	...
	wp_register_script( 'customize-script', get_stylesheet_directory_uri() . '/admin/js/customize-script.js', array( 'jquery' ), '3.0', true );
...
<fieldset class="smartadapt-fieldset">
	...
	<h4><?php _e( 'Category layout', 'smartadapt' ); ?></h4>
...
add_action( 'edited_category', 'smartadapt_save_extra_category_fileds' );
add_action( 'created_category', 'smartadapt_save_extra_category_fileds' );

// save extra category extra fields callback function
function smartadapt_save_extra_category_fileds( $term_id ) {
...
<fieldset class="smartadapt-fieldset">
	<h4><?php _e( 'Header Banner', 'smartadapt' ) ?></h4>
...
/**
 * add smartadapt custom header save hook
 */
function smartadapt_banner_code_header_save() {
	if ( isset( $_POST['banner_code_header'] ) ) {
		//check_admin_referer( 'banner_code_header_option', '_wpnonce_banner_code_header_option' );
		if ( current_user_can( 'manage_options' ) ) {
			set_theme_mod( 'banner_code_header', stripslashes( $_POST['banner_code_header'] ) );
		}
	}

	if ( isset( $_POST['banner_display_subpages'] ) ) {
		if ( current_user_can( 'manage_options' ) ) {
			set_theme_mod( 'banner_display_subpages', stripslashes( $_POST['banner_display_subpages'] ) );
		}
	}
	return;
}

add_action( 'admin_head', 'smartadapt_banner_code_header_save' );